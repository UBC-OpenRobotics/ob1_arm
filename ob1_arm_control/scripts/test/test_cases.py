...
################### UNIT TESTS ######################
class UnitTests:
    @pytest.mark.skip("not using pyquaternion anymore")
...
@pytest.mark.skip("not using this iksolver anymore")
class TestIKPoints:
    ...
    def test_pick_and_place_cylinder(self, arm_commander, log, setup_teardown,attempts, pick_tolerance, place_tolerance):
        p = PoseStamped()
        p.header.frame_id = arm_commander.robot.get_planning_frame()
        p.pose.position = arm_commander.arm_mvgroup.get_random_pose().pose.position
        p.pose.orientation.w = 1
    ...
    def test_pick_and_place_cylinder_in_region(self, arm_commander, log, setup_teardown):
